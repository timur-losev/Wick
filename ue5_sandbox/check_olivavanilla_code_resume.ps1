# ============================================================
#  OlivaVanilla code resume diff check
#  Compares the current file tree against the cached file manifest.
#  No indexing is started here.
# ============================================================

param(
    [string]$ProjectRoot = 'J:\UE4\Projects\OlivaVanilla',
    [string]$CheckpointNamespace = 'olivavanilla_code',
...
$projectRootForward = $projectRootFull.TrimEnd([System.IO.Path]::DirectorySeparatorChar).Replace('\', '/')
...
Write-Host '============================================================'
Write-Host ' OlivaVanilla code resume diff check'
...
if ($checkpointRepoRoot -ne $projectRootForward) {
    Write-Host 'Check failed: checkpoint repo_root does not match this project.' -ForegroundColor Red
    Write-Host "  expected:  $projectRootForward"
...
Write-Host "Preflight ok: checkpoint matches $projectRootForward"
Write-Host ''
...
$currentItems = Get-CurrentFiles -RootPath $projectRootFull -IncludeExtensions @('.h', '.hpp', '.cpp', '.inl', '.inc') -ExcludeDirs @('DerivedDataCache', 'Intermediate', 'Saved', 'Binaries', 'Build', 'Content')
